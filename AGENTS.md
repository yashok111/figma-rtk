...
Codex and the **remote** Figma MCP server and compresses the heavy read-tool
responses before they reach the agent:

```
Codex  ──►  frtk (localhost:7337)  ──►  https://mcp.figma.com/mcp
```
...
(`~/.Codex/projects/-home-yakov-projects-Codex-figma/memory/figma-rtk-project.md`)
...
2. **OAuth discovery must be rewritten, or auth fails.** Codex's SDK does a
   ...
   `api.figma.com` (so OAuth still runs agent↔Figma directly; the proxy never sees
   the token exchange). **Agents cache discovery from first connect** — after
   changing auth behavior, reconnect the Figma MCP server or restart Codex.
...
- **Two Figma integrations may be connected:** `mcp__codex_apps__figma__*` is the
  Codex Apps connector (**unproxied** — bypasses frtk), while the local Codex MCP
  server configured by `frtk init` is expected to expose tools such as
  `mcp__figma__get_design_context`. Any capture / proxied read MUST use the local
  MCP server tools, not the app-backed connector.
- For Codex config, `CODEX_HOME` wins absolutely: when set, `frtk init/status`
  operate on `$CODEX_HOME/config.toml` even if the file does not exist yet.
  `frtk init --file <path>` and `frtk status --file <path>` are the paired escape
  hatch for non-default config paths.
